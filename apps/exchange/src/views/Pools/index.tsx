...
const sortPools = (account: string, sortOption: string, pools: DeserializedPool[], poolsToSort: DeserializedPool[]) => {
  switch (sortOption) {
        ...
        (pool: DeserializedPool) => {
          ...
          if (pool.vaultKey) {
            ...
            if (pool.stakingTokenPrice && vault.totalWayaInVault.isFinite()) {
              totalStaked =
                +formatUnits(EthersBigNumber.from(vault.totalWayaInVault.toString()), pool.stakingToken.decimals) *
    ...
    case 'latest':
      return orderBy(poolsToSort, (pool: DeserializedPool) => Number(pool.poolId), 'desc')
    default:
      return poolsToSort
  }
}
    ...
    <CardLayout>
      {chosenPools.map((pool) =>
        pool.vaultKey ? (
          ...
          <PoolCard key={pool.poolId} pool={pool} account={account} />
        ),
      )}
    </CardLayout>
  )
        ...
        <PoolControls>
          ...
          <FilterContainer>
            ...
            <LabelWrapper style={{ marginLeft: 16 }}>
              ...
              <SearchInput initialValue={searchQuery} onChange={handleChangeSearchQuery} placeholder="Search Silos" />
            </LabelWrapper>
          </FilterContainer>
        </PoolControls>
        {showFinishedPools}
