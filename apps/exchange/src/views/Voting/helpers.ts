...
import { ADMINS, PLEX_SPACE, SNAPSHOT_VERSION } from './config'
...
const STRATEGIES = [
  { name: 'waya', params: { symbol: 'WAYA', address: bscTokens.waya.address, decimals: 18, max: 300 } },
]
const NETWORK = '56'
...
export const getVotingPower = async (
  ...
  if (blockNumber && (blockNumber >= VOTING_POWER_BLOCK.v0 || blockNumber >= VOTING_POWER_BLOCK.v1)) {
    ...
    const [
      ...
      PLEX_SPACE,
      [
        strategies.wayaBalanceStrategy(version),
        strategies.wayaBnbLpBalanceStrategy(version),
        strategies.wayaPoolBalanceStrategy(version),
        strategies.wayaVaultBalanceStrategy(version),
        strategies.createPoolsBalanceStrategy(poolAddresses, version),
        strategies.createTotalStrategy(poolAddresses, version),
        strategies.lockedWaya(wayaVaultAddress, 'lockedAmount'),
        strategies.lockedWaya(wayaVaultAddress, 'lockEndTime'),
    ...
    return {
      ...
      wayaBalance: wayaBalance[account] ? wayaBalance[account] : 0,
      wayaPoolBalance: wayaPoolBalance[account] ? wayaPoolBalance[account] : 0,
      wayaBnbLpBalance: wayaBnbLpBalance[account] ? wayaBnbLpBalance[account] : 0,
      wayaVaultBalance: wayaVaultBalance[account] ? wayaVaultBalance[account] : 0,
      lockedWayaBalance: lockedWayaBalance[account]
        ? new BigNumber(lockedWayaBalance[account]).div(BIG_TEN.pow(18)).toNumber()
        : 0,
      lockedEndTime: lockedEndTime[account] ? lockedEndTime[account] : 0,
    }
  }

  const [total] = await getScores(PLEX_SPACE, STRATEGIES, NETWORK, [account], blockNumber)
 ...
 */
export async function getVotingPowerByWayaStrategy(voters: string[], blockNumber: number) {
  const strategyResponse = await getScores(PLEX_SPACE, STRATEGIES, NETWORK, voters, blockNumber)
