...
import { getVotingPowerByWayaStrategy } from 'views/Voting/helpers'
    ...
    gql`
      query getProposals($first: Int!, $skip: Int!, $state: String!, $orderDirection: OrderDirection) {
        proposals(
          ...
          where: { space_in: "wayavote.eth", state: $state }
        ) {
          id
          title
          body
...
export const getAllVotes = async (proposal: Proposal, votesPerChunk = 30000): Promise<Vote[]> => {
  ...
  const vps = await Promise.all(voterChunk.map((v) => getVotingPowerByWayaStrategy(v, parseInt(proposal.snapshot))))
