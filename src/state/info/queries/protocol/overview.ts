...
const formatPlexFactoryResponse = (rawPlexFactory?: PlexFactory) => {
  if (rawPlexFactory) {
    return {
      totalTransactions: parseFloat(rawPlexFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawPlexFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawPlexFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatPlexFactoryResponse(data?.plexFactories?.[0])
      const overviewData24 = formatPlexFactoryResponse(data24?.plexFactories?.[0])
      const overviewData48 = formatPlexFactoryResponse(data48?.plexFactories?.[0])
