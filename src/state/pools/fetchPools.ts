...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingResultRaw = await multicallv2(TaskAssistantV2, poolStakingCalls, { requireSuccess: false })
  ...
  return poolStakingChunkedResultRaw.reduce((accum, stakingLimitRaw, index) => {
    ...
    return {
      ...accum,
      [validPools[index].poolId]: { stakingLimit, numberBlocksForUserLimit },
    }
  }, {})
}
...
export const fetchPoolsProfileRequirement = async (): Promise<{
  ...
  const poolProfileRequireResultRaw = await multicallv2(TaskAssistantV3, poolProfileRequireCalls, { requireSuccess: false })
