...
const LockedStakeModal: React.FC<React.PropsWithChildren<GenericModalProps>> = ({
  ...
  const usdValueStaked = useBUSDWayaAmount(_toNumber(lockedAmount))

  return (
    <RoiCalculatorModalProvider lockedAmount={lockedAmount}>
      <Modal title={t('Lock WAYA')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
        <Box mb="16px">
          <BalanceField
