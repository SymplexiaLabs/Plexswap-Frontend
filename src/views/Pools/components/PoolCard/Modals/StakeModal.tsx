...
const StakeModal: React.FC<React.PropsWithChildren<StakeModalProps>> = ({
  ...
  const { poolId, stakingToken, earningTokenPrice, apr, userData, stakingLimit, earningToken } = pool
  const { account } = useWeb3React()
  const dispatch = useAppDispatch()
  const { t } = useTranslation()
  const { theme } = useTheme()
  const { onStake } = useStakePool(poolId, isBnbPool)
  const { onUnstake } = useUnstakePool(poolId, pool.enableEmergencyWithdraw)
