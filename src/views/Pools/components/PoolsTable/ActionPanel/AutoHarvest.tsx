...
const AutoHarvestAction: React.FunctionComponent<React.PropsWithChildren<DeserializedPool>> = ({
  ...
  const {
    userData: { userShares, wayaAtLastUserAction },
    pricePerFullShare,
  } = vaultData
  const { hasAutoEarnings, autoWayaToDisplay, autoUsdToDisplay } = getWayaVaultEarnings(
    ...
    vaultKey === VaultKey.WayaVault
      ? (vaultData as DeserializedLockedWayaVault).userData.currentPerformanceFee
          .plus((vaultData as DeserializedLockedWayaVault).userData.currentOverdueFee)
          .plus((vaultData as DeserializedLockedWayaVault).userData.userBoostedShare)
      : null,
  )
