...
import AddWayaButton from '../../LockedPool/Buttons/AddWayaButton'
...
const Staked: React.FunctionComponent<React.PropsWithChildren<StackedActionProps>> = ({ pool }) => {
  ...
  const { lockEndDate, remainingTime, burnStartTime } = useUserDataInVaultPresenter({
    ...
    burnStartTime:
      vaultKey === VaultKey.WayaVault ? (vaultData as DeserializedLockedWayaVault).userData?.burnStartTime ?? '0' : '0',
  })
  ...
  if (isNotVaultAndHasStake || isVaultWithShares) {
    ...
    return (
      <>
          ...
          <ActionContent mt={0}>
            <Flex flex="1" flexDirection="column" alignSelf="flex-start">
              <ActionTitles>
                ...
                <Text fontSize="12px" bold color="textSubtle" as="span" textTransform="uppercase">
                  {vaultKey === VaultKey.WayaVault && (vaultData as DeserializedLockedWayaVault).userData.locked
                    ? t('Locked')
                    : t('Staked')}
                </Text>
              </ActionTitles>
              ...
              <Flex flex="1" ml="20px" flexDirection="column" alignSelf="flex-start">
                ...
                <Text lineHeight="1" mt="8px" bold fontSize="20px" color="failure">
                  {vaultPosition === VaultPosition.AfterBurning ? (
                    isUndefinedOrNull((vaultData as DeserializedLockedWayaVault).userData.currentOverdueFee) ? (
                      '-'
                    ) : (
                      t('%amount% Burned', {
                        amount: getFullDisplayBalance(
                          (vaultData as DeserializedLockedWayaVault).userData.currentOverdueFee,
                          18,
                          5,
                        ),
                      })
                    ...
                    <BurningCountDown lockEndTime={(vaultData as DeserializedLockedWayaVault).userData.lockEndTime} />
                  )}
                </Text>
              </Flex>
            )}
        ...
        {vaultKey === VaultKey.WayaVault && vaultPosition === VaultPosition.Flexible && (
          <Box
            width="100%"
            mt={['0', '0', '24px', '24px', '24px']}
            ...
            mr={['0', '0', '12px', '12px', '0']}
          >
            <ConvertToLock stakingToken={stakingToken} currentStakedAmount={wayaAsNumberBalance} isInline />
          </Box>
        )}
      </>
    )
      ...
      <ActionContent>
        {vaultKey ? (
          <VaultStakeButtonGroup
            ...
            onLockedClick={vaultKey === VaultKey.WayaVault ? openPresentLockedStakeModal : null}
          />
        ) : (
          <Button
            width="100%"
