import { Box, Flex, Skeleton, Text, useMatchBreakpointsContext } from '@plexswap/ui-plex'
...
const StakedCell: React.FC<React.PropsWithChildren<StakedCellProps>> = ({ pool, account }) => {
  ...
  const labelText = `${pool.stakingToken.symbol} ${
    pool.vaultKey === VaultKey.WayaVault && (vaultData as DeserializedPoolLockedVault).userData.locked
      ? t('Locked')
      : t('Staked')
  }`
  ...
  return (
    <StyledCell
      role="cell"
      flex={
        pool.vaultKey === VaultKey.WayaFlexibleVault
          ? '1 0 162px'
          : pool.vaultKey === VaultKey.WayaVault && !hasStaked
          ? '1 0 120px'
          : '2 0 100px'
      }
    >
