...
import { Flex, Box, SwapVertIcon, IconButton } from '@plexswap/ui-plex'
  ...
  return (
    <div ref={observerRef}>
      <Flex flexDirection="column" mt="24px">
        <Flex mb="24px">
          <RowHeading text={showFarms ? t('Top Farms') : t('Top Crop Silos')} />
          <IconButton
            variant="text"
            height="100%"
            width="auto"
          ...
          <Grid>
            {topPools.map((topPool, index) => (
              <TopFarmPool
                ...
                percentage={topPool?.poolId === 0 ? +lockedApy : topPool?.apr}
                index={index}
                isApy={topPool?.poolId === 0}
                visible={!showFarms}
              />
            ))}
          </Grid>
