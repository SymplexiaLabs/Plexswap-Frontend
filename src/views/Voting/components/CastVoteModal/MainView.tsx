...
const MainView: React.FC<React.PropsWithChildren<MainViewProps>> = ({
  ...
  const hasLockedWaya = lockedWayaBalance > 0
  ...
  const hasBoosted = hasLockedWaya && !isBoostingExpired
          ...
          <>
            ...
            {total === 0 ? (
              <Message variant="danger" mb="12px">
                <Text color="danger">
                  {t(
                    'Hold some WAYA in your wallet or on PlexSwap at the snapshot block to get voting power for future proposals.',
                  )}
                </Text>
              </Message>
            ) : (
