...
import { AutoRenewIcon } from '@plexswap/ui-plex'
    ...
    switch (boosterState) {
      ...
      case YieldBoosterState.NO_LOCKED:
        return (
          <ActionButton
            title={`${t('Up to')} ${boostMultiplerDisplay}x`}
            description={t('Lock WAYA to activate yield booster')}
      ...
      case YieldBoosterState.LOCKED_END:
        return (
          <ActionButton
            title={`${t('Up to')} ${boostMultiplerDisplay}x`}
            description={t('Lock WAYA is ended. Re-lock WAYA to activate yield booster')}
            style={{ whiteSpace: 'nowrap' }}
          >
            <NextLinkFromReactRouter to="/pools">{t('Go to Pool')}</NextLinkFromReactRouter>
          </ActionButton>
