import { ChainId, Currency, CurrencyAmount, JSBI, Pair, Price, Token, WNATIVE } from '@plexswap/sdk'
...
export const useWayaBusdPrice = (): Price<Currency, Currency> | undefined => {
  const { chainId } = useActiveWeb3React()
  // Default bsc waya, if no waya address found
  const waya: Token = WAYA[chainId] ? WAYA[chainId] : WAYA[ChainId.BSC]
