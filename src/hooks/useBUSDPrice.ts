import { Currency, JSBI, Price, WNATIVE, ChainId } from '@plexswap/sdk'
import { WAYA, BUSD } from 'config/constants/tokens'
...
export const useWayaBusdPrice = (): Price<Currency, Currency> | undefined => {
  const { chainId } = useActiveWeb3React()
  const wayaBusdPrice = useBUSDPrice(WAYA[chainId])
  return wayaBusdPrice
}
